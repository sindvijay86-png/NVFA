...
export default function App() {
  ...
  useEffect(() => {
    const code = localStorage.getItem("nvfc_code");
    if (code) db("login").then((r) => setCoach(r.coach)).catch(() => localStorage.removeItem("nvfc_code"));
  }, []);

  const logout = () => { localStorage.removeItem("nvfc_code"); localStorage.removeItem("nvfc_name"); setCoach(null); };
  ...
  return (
    <div className="body" style={{ minHeight: "100vh", background: C.pitch, display: "flex", justifyContent: "center" }}>
      ...
      <div style={{ width: "100%", maxWidth: 440, display: "flex", flexDirection: "column", minHeight: "100vh", position: "relative" }}>
        {!coach && <Login onIn={setCoach} />}
        {coach && (
          <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px 0" }}>
              <span className="display" style={{ color: C.chalk, fontSize: 20 }}>NVFC</span>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <Tag color={C.grass}>{coach.name}</Tag>
                <button onClick={logout} className="body" style={{ background: "none", border: "none", color: C.chalkDim, fontSize: 12, cursor: "pointer" }}>बाहर</button>
              </div>
            </div>
